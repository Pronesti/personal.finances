...
import {
  loadCategoryFile, saveCategoryFile, upsertRule, CATEGORIES, type Category,
} from "@/lib/categorize";

// Correcting a charge's category from the drill table. A single transaction cannot own a category
// of its own: rows are rebuilt from the rules file on every ingest and their ids do not survive it
...
export async function recategorizeAction(formData: FormData): Promise<void> {
  ...
  const data = upsertRule(loadCategoryFile(), {
  ...
  saveCategoryFile({ ...data, proposals: data.proposals.filter(p => p.merchant !== merchant) });
  recategorize(getDb(), match, category, subcategory || null, "all");
