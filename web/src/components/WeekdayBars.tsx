  ...
  return (
    <ResponsiveContainer width="100%" height={420}>
      <ComposedChart data={data}>
        ...
        {categories.map((c, i) => (
          <Bar key={c} yAxisId="amt" dataKey={c} stackId="1" fill={t.category[c]}
            radius={i === categories.length - 1 ? [3, 3, 0, 0] : undefined} />
        ))}
        <Line yAxisId="n" dataKey="count" name="purchases" stroke={t.ink} strokeWidth={2} strokeDasharray="5 4" dot />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
